...
import os
from ollama_curl import get_ollama_response
...
class Crawler:
	...
	def type(self, id, text):
		print("type_command executed:", self, id, text)
		self.click(id)
...
if (
	...
	def get_gpt_command(objective, url, previous_command, browser_content):
		...
		#response = get_ollama_response(prompt, "phi3-browser")
		response = get_ollama_response(prompt, "phi3-browser")
		print("response string that has been recived: ", response["response"])
		response_list = response["response"].split("\n")
		response_filtered = ""
		for line in response_list:
			print("this line is going trough")
			if "SCROLL" in line:
				filtered_line = re.findall(r"""SCROLL\s+\b[^,"\s]+\b""", line)[0]
				response_filtered = response_filtered + filtered_line +"\n"
			if "CLICK" in line:
				filtered_line = re.findall(r"""CLICK\s+\d""", line)[0]
				response_filtered = response_filtered + filtered_line +"\n"
			if "TYPE " in line:
				filtered_line = re.findall(r'TYPE\s+\d+\s+"[^"]+"', line)
				response_filtered = response_filtered + filtered_line[0] +"\n"
			if "TYPESUBMIT" in line:
				print(line)
				try:
					filtered_line = re.findall(r'TYPESUBMIT\s+\d+\s+"[^"]+"', line)[0]
				except:
					filtered_line = "TYPESUBMIT "+line.split("TYPESUBMIT")[1]
				response_filtered = response_filtered + filtered_line +"\n"

		
		return response_filtered#.choices[0].text
